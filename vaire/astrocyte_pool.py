...
class AstrocytePool:
    ...
    def consolidate_domain(self, process_name: str) -> dict:
        ...
        for mid in memory_ids:
            ...
            # Collect entity IDs for this specific memory so we can build
            # co_occurrence edges between entities that appear together.
            mem_entity_ids: list[int] = []
            for name, etype, _ctx in typed_entities:
                if etype in entity_types:
                    ...
                    mem_entity_ids.append(eid)
                    if eid not in entity_ids:
                        entity_ids.append(eid)
                        stats["entities_extracted"] += 1

            # Build co_occurrence edges for entities that share this memory.
            for id_a, id_b in combinations(mem_entity_ids, 2):
