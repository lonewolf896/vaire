...
class TestTransportContext:
    ...
    def test_set_remote(self):
        info = TransportInfo(is_remote=True, agent_cn="halcyon", client_ip="100.64.0.1")
        token = transport_ctx.set(info)
        try:
            current = transport_ctx.get()
            assert current.is_remote
            assert current.agent_cn == "halcyon"
            assert current.client_ip == "100.64.0.1"
        finally:
...
class TestMTLSMiddleware:
    ...
    def test_middleware_sets_context(self):
        ...
        async def _run():
            ...
            scope = {
                "type": "http",
                "headers": [(b"x-vaire-cn", b"halcyon")],
        ...
        assert captured["agent_cn"] == "halcyon"
    ...
    def test_middleware_case_insensitive_header(self):
        ...
        async def _run():
            ...
            scope = {
                "type": "http",
                "headers": [(b"X-Vaire-CN", b"halcyon-mixed-case")],
                "client": ("127.0.0.1", 1234),
            }
            await middleware(scope, None, None)

        asyncio.run(_run())
        assert captured["cn"] == "halcyon-mixed-case"
