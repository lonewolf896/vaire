...
class TestCooldownGuard:
    def test_cooldown_prevents_reconsolidation_within_window(self, engine):
        """Memory reconsolidated recently should return 'none' despite high mismatch."""
        now_iso = datetime.now(timezone.utc).isoformat()
        memory = {
            "stability": 0.0,
            "plasticity": 0.8,
            "is_protected": False,
            "last_reconsolidated": now_iso,  # just reconsolidated
        }
        # High mismatch would normally trigger archive, but cooldown blocks it
        result = engine.should_reconsolidate(memory, 0.9)
        assert result == "none"

    def test_cooldown_allows_reconsolidation_after_window(self, engine):
        """Memory reconsolidated longer ago than cooldown should be eligible."""
        past = (datetime.now(timezone.utc) - timedelta(hours=48)).isoformat()
        memory = {
            "stability": 0.0,
            "plasticity": 0.8,
            "is_protected": False,
            "last_reconsolidated": past,  # 48h ago, well outside 24h cooldown
        }
        result = engine.should_reconsolidate(memory, 0.9)
        assert result == "archive"

    def test_cooldown_null_last_reconsolidated_allows_update(self, engine):
        """Memory never reconsolidated (null) should not be blocked by cooldown."""
        memory = {
            "stability": 0.0,
            "plasticity": 0.0,
            "is_protected": False,
