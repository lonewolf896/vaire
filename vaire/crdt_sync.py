...
class CRDTMemorySync:
    ...
    def tag_write(self, memory_id: int, operation: str) -> None:
        if not self._active_agent_id:
            raise RuntimeError("tag_write called before set_active_agent")
        clock = self._increment_clock_for_agent(self._active_agent_id)
        self._storage.upsert_crdt_entry(
            memory_id=memory_id,
            agent_id=self._active_agent_id,
            operation=operation,
            vector_clock=json.dumps(clock),
    ...
    def resolve_conflict(self, memory_id: int, strategy: str = "latest") -> dict:
        ...
        if updated is None:
            return {"memory_id": memory_id, "_resolved": True, "_strategy": strategy}
    ...
    def sync_memories(self, remote_memories: list[dict]) -> dict:
        ...
        for remote in remote_memories:
            ...
            if local_match is None:
                ...
                # Set provenance fields via the public storage API, preserving
                # the remote memory's last_accessed timestamp so it is not
                # overwritten with the local insertion time.
                self._storage.update_memory_full(
                    mid,
                    provenance_agent=remote.get("provenance_agent", "unknown"),
                    vector_clock=remote.get("vector_clock", "{}"),
                    last_accessed=remote.get("last_accessed") or None,
                )
                ...
                if relation == "equal":
                    ...
                    # Write merged content back via the public storage API.
                    self._storage.update_memory_full(
                        local_match["id"],
