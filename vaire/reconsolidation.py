...
class ReconsolidationEngine:
    ...
    def compute_mismatch(
        ...
        # Signal 4: Tag divergence — only meaningful when both sides have structured tags.
        # Raw query strings are word tokens, not tags; comparing them to semantic labels
        # (e.g. "ansible", "bugfix") yields near-100% divergence for every recall and
        # artificially inflates mismatch.  Use neutral (0.5) unless both sides have tags.
        raw_tags = memory.get("tags") or []
        memory_tags = set(raw_tags.split(",") if isinstance(raw_tags, str) else raw_tags)
        memory_tags = {t.strip() for t in memory_tags if t.strip()}
        tag_divergence = 0.5  # neutral fallback

        # Weighted sum (tag weight redistributed to embedding since signal is unreliable)
        mismatch = (
    ...
    def should_reconsolidate(self, memory: dict, mismatch: float) -> str:
        ...
        # Cooldown guard: don't reconsolidate the same memory more than once per window
        last_recon_str = memory.get("last_reconsolidated")
        if last_recon_str:
            try:
                last_recon = datetime.fromisoformat(last_recon_str) if isinstance(last_recon_str, str) else last_recon_str
                if last_recon.tzinfo is None:
                    last_recon = last_recon.replace(tzinfo=timezone.utc)
                hours_since = (datetime.now(timezone.utc) - last_recon).total_seconds() / 3600.0
                if hours_since < self._settings.RECONSOLIDATION_COOLDOWN_HOURS:
                    return "none"
            except (ValueError, TypeError):
                pass
