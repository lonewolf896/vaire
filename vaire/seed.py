"""Project seeding — bootstrap Vaire memory for existing codebases.
...
def _delete_existing_seed_memories(storage) -> int:
    """Delete existing _seed tagged memories for the directory being re-seeded.
...
def seed_project(
    ...
    Args:
        ...
        db_path: Optional SQLite database path override (only used when storage=None).
        dry_run: If True, scan and generate but don't store.
        storage: Optional pre-initialized StorageEngine (server reuses its own).
    ...
    Returns:
        Dict with scan stats and counts of memories created/replaced.
    """
    ...
    try:
        deleted = _delete_existing_seed_memories(storage)
        if deleted:
            logger.info("Cleared %d existing seed memories", deleted)
            replaced = deleted
        ...
        for mem in memories:
            ...
            # insert_memory handles FTS, vec0, and enrichment automatically
