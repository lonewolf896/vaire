...
class DualStoreCLS:
    ...
    def find_recurring_patterns(
        ...
        for cluster in clusters:
            ...
            # Go-CLS: require generalizability across contexts.
            # For live-session memories: >= 2 different sessions.
            # For bulk-ingested memories (no source_episode_id): >= 2 different
            # directories serves as the diversity signal instead of sessions.
            has_episode_link = any(m.get("source_episode_id") for m in cluster)
            if has_episode_link:
                if len(session_ids) < 2:
                    continue
            else:
                # Ingested memories: accept if from >= 2 directories
                if len(directories) < 2 and len(session_ids) < 2:
                    continue
