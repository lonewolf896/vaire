"""Vaire MCP server — supports SSE and Streamable HTTP transports."""
...
from vaire.fractal import FractalMemoryTree
from vaire.hdc_encoder import HDCEncoder
from vaire.hopfield import HopfieldMemory
...
from vaire.predictive_coding import PredictiveCodingGate
from vaire.prospective import ProspectiveMemoryEngine
from vaire.reconsolidation import ReconsolidationEngine
from vaire.retrieval import HippoRetriever
from vaire.rules_engine import RulesEngine
from vaire.sensory_buffer import SensoryBuffer
from vaire.sleep_compute import SleepComputeEngine
from vaire.staleness import StalenessDetector
...
_groomer = None   # GroomerEngine — imported lazily in init_engines
...
@mcp_server.tool()
def remember(content: str, context: str, tags: list[str], force: bool = False) -> dict:
    ...
    elif _write_gate is not None and force:
        _, surprisal, reason = _write_gate.should_store(content, context, tags)
        gate_result = {
            "surprisal": round(surprisal, 4),
    ...
    if curator is not None and embedding is not None:
        try:
            ...
            logger.exception("Curator failed; falling back to direct insert")
            curator = None  # trigger the else path below

    if curator is None or embedding is None:
    ...
    if _hdc is not None:
        try:
            from vaire.retrieval import _extract_query_entities
    ...
    if memory is None:
        # Memory was just created; a concurrent delete is possible but very rare.
        return {"memory_id": memory_id, "status": "stored", "curation_action": curation_action}
...
def memory_stats() -> dict:
    ...
    # Count causal edges from both sources:
...
def rate_memory(memory_id: int, rating: float = 1.0, was_useful: bool | None = None) -> dict:
    ...
    # Accept either rating (float 0-1) or was_useful (bool); derive the other.
    if was_useful is None:
        was_useful = rating >= 0.5
    ...
    # Fall back to the pre-access snapshot if the memory was concurrently deleted.
    updated = storage.get_memory(memory_id) or mem
    return {
        "memory_id": memory_id,
        "status": "rated",
...
def seed_project(directory: str, dry_run: bool = False) -> dict:
    ...
    - Config files (package.json, pyproject.toml, Cargo.toml, go.mod, Dockerfile, etc.)
    - Documentation (README, ARCHITECTURE, CONTRIBUTING, CLAUDE.md, etc.)
    - CI/CD configuration (.github/workflows, .gitlab-ci.yml, Makefile, etc.)
    - Entry points and key source files (main.*, server.*, __main__.py, etc.)
...
def sync_instructions(claude_md_path: str = "") -> dict:
    ...
    Finds or creates the '## Memory System — Vaire' section in CLAUDE.md
    and updates it with the latest tools, capabilities, and rules.
    Call this on session start or after Vaire updates.
    ...
    # The canonical Vaire section
    vaire_section = f"""## Memory System — Vaire v{__version__}
- ALWAYS use the Vaire MCP tools (remember, recall, get_project_context) for memory operations
- On EVERY new session start, call `recall` with the current project name to load prior context
- NEVER rely on CLAUDE.md or built-in memory for cross-session context — use Vaire
...
- Decisions are auto-protected from decay/compression when detected"""
    ...
    if md_path.exists():
        ...
        # Find and replace existing Vaire section
        import re
        # Match from "## Memory System" to next "## " header or end of file
        pattern = r"## Memory System — Vaire[^\n]*\n(?:(?!## )[^\n]*\n)*"
        if re.search(pattern, content):
            new_content = re.sub(pattern, vaire_section + "\n\n", content)
        else:
    ...
    return {
        ...
        "path": str(md_path),
        "version": __version__,
        "section_length": len(vaire_section),
    }
...
def build_dispatch_table() -> dict:
    """Build the socket server dispatch table from all registered MCP tool functions.

    Each handler accepts ``agent_id`` (stripped before forwarding) plus the
    original tool's keyword arguments.  CPU/IO-heavy tools run in a thread-pool
    executor so they do not block the asyncio event loop.
    """
    tools = [
        remember, recall, forget, get_project_context, consolidate_now,
        memory_stats, rate_memory, validate_memory, recall_hierarchical,
    ...
    _executor_tools = frozenset({"consolidate_now", "recall", "remember", "restore", "seed_project"})
    ...
    return {fn.__name__: _make_wrapper(fn) for fn in tools}


def build_groomer_dispatch() -> dict:
    """Build groomer-only dispatch table from GroomerEngine methods."""
    if _groomer is None:
        return {}
    g = _groomer

    async def groom_audit(agent_id: str = "", **p):
        return g.audit(**p)

    async def groom_inspect(agent_id: str = "", **p):
        return g.inspect(**p)

    async def groom_duplicates(agent_id: str = "", **p):
        return g.find_duplicates(**p)

    async def groom_contradictions(agent_id: str = "", **p):
        return g.find_contradictions(**p)

    async def groom_orphans(agent_id: str = "", **p):
        return g.find_orphans(**p)

    async def groom_stale(agent_id: str = "", **p):
        return g.find_stale(**p)

    async def groom_stats(agent_id: str = "", **p):
        return g.get_stats(**p)

    async def groom_merge(agent_id: str = "", **p):
        return g.merge(agent_id=agent_id, **p)

    async def groom_split(agent_id: str = "", **p):
        return g.split(agent_id=agent_id, **p)

    async def groom_retag(agent_id: str = "", **p):
        return g.retag(**p)

    async def groom_reclassify(agent_id: str = "", **p):
        return g.reclassify(**p)

    async def groom_update_content(agent_id: str = "", **p):
        return g.update_content(**p)

    async def groom_promote(agent_id: str = "", **p):
        return g.promote(**p)

    async def groom_demote(agent_id: str = "", **p):
        return g.demote(**p)

    async def groom_bulk_delete(agent_id: str = "", **p):
        return g.bulk_delete(**p)
    ...
    return {
        ...
        "groom_duplicates": groom_duplicates,
        "groom_contradictions": groom_contradictions,
        "groom_orphans": groom_orphans,
        "groom_stale": groom_stale,
        "groom_stats": groom_stats,
        "groom_merge": groom_merge,
        "groom_split": groom_split,
        "groom_retag": groom_retag,
        "groom_reclassify": groom_reclassify,
        "groom_update_content": groom_update_content,
        "groom_promote": groom_promote,
...
def build_ingest_dispatch(pipeline) -> dict:
    """Build ingest dispatch table from IngestionPipeline methods.

    The socket server calls ``handler(**params, agent_id=...)``.  These
    wrappers collect all non-agent_id kwargs into *params* and forward them
    to the pipeline using its ``(params: dict, agent_id: str)`` signature.

    ingest_file and ingest_directory run embedding generation (CPU-bound) so
    they are dispatched via run_in_executor to avoid blocking the event loop.
    ingest_directory additionally returns immediately with a job_id and runs
    the full crawl in the background so the caller never times out.
    """
    import asyncio as _asyncio
    import uuid as _uuid

    # Tracks background directory ingest jobs: job_id -> result dict or None
    _dir_jobs: dict = {}

    async def ingest_file(agent_id: str = "", **params):
        return await pipeline.ingest_file(params, agent_id)
    ...
    async def ingest_directory(
        ...
        """Return a job_id immediately; run the full ingest in the background.

        project_dir: directory_context to record for all chunks (host-side path).
            If omitted each chunk uses its source file path as context.
            Example: '/home/user/workspace/myproject'
        tags: list of tags to attach to every ingested chunk.
        """
        params = {
    ...
    async def ingest_status(agent_id: str = "", **params):
        job_id = params.get("job_id", "")
        # Check background directory jobs first
        if job_id in _dir_jobs:
    ...
    async def ingest_preview(agent_id: str = "", **params):
        return await pipeline.ingest_preview(params, agent_id)
...
def init_engines(
    ...
    # Phase 6/7: cache, write queue, and groomer (lazy imports avoid circular deps)
    from vaire.cache import MemoryCache
...
async def async_shutdown() -> None:
    """Async pre-shutdown: drain the write queue before tearing down engines.

    Must be awaited from an async context (e.g. cmd_server's finally block)
    before calling the synchronous shutdown().  Does NOT null _write_queue so
    that shutdown()'s sync fallback can still see the queue reference.
    """
...
def shutdown():
    ...
    if _write_queue is not None:
        ...
        # When called after async_shutdown(), _running is already False and
        # _task is already None — these lines are safe no-ops in that case.
        _write_queue._running = False
        if _write_queue._task is not None:
            _write_queue._task.cancel()
        try:
            _write_queue._drain_sync()
        except Exception:
            pass
    if _consolidation is not None:
        _consolidation.stop()
...
def main(
    ...
    try:
        sync_instructions()
        logger.info("CLAUDE.md synced with Vaire v%s", __version__)
    except Exception:
